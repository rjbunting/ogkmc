...
from ogkmc.io.extxyz import read_atoms as ase_read, write_extxyz as ase_write

from ogkmc.io._files import atomic_output_path, write_json_atomic
...
from ogkmc.utils.telemetry import increment, instrument
...
REACTION_DATABASE_SCHEMA = "ogkmc-reaction-database-v2"
...
_GEOMETRY_FINGERPRINT_SCHEMA = "ogkmc-local-geometry-v2"
_SCIENTIFIC_INPUT_FINGERPRINT_SCHEMA = "ogkmc-scientific-input-v3"
_INPUT_FRAME_FINGERPRINT_SCHEMA = "ogkmc-input-coordinate-frame-v2"

# These values identify an enumeration in one OGKMC run, not a scientific
...
def _scientific_input_payload(
    ...
    *,
    ...
    for key in _SEMANTIC_INPUT_FIELDS:
        ...
        if value not in (None, ""):
            from ogkmc.species.smiles import canonical_smiles
...
def calculation_cache_key(
    *,
    ...
    from ogkmc.species.smiles import SMILES_IDENTITY_VERSION
...
def _safe_atoms_copy(atoms: Atoms, energy_ev: float | None = None) -> Atoms:
    ...
    if energy_ev is not None:
        snapshot.info["ogkmc_energy_ev"] = float(energy_ev)
    return snapshot
...
def _calculator_method(parameters: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    method = {
        "family": family,
        "code": class_name,
        "notes": "Calculator declaration is mirrored in system.configuration.ogkmc.calculator.",
    }
...
def _package_version() -> str:
    try:
        return importlib_metadata.version("ogkmc")
    except importlib_metadata.PackageNotFoundError:
        return "unknown"
...
def _build_isaac_record(
    ...
    *,
    ...
    if run_id is not None:
        ogkmc_configuration["run_id"] = str(run_id)

    descriptors = [
        _descriptor("ogkmc.reaction_kind", kind, kind="categorical"),
        _descriptor("ogkmc.graph_hash", graph_hash, kind="categorical"),
    ...
    record: dict[str, Any] = {
        ...
        "system": {
            ...
            "technique": technique,
            "configuration": {"ogkmc": ogkmc_configuration},
        },
        "computation": {
            "method": method,
            "output_quantity": (
        ...
        "tags": ["ogkmc-reaction-database", f"ogkmc-{kind}"],
    }
...
def _isaac_validator():
    ...
    schema_path = resource_files("ogkmc").joinpath("schema/isaac_record_v1.json")

...
ogkmc.io.config
==============
Configuration loader for the :mod:`ogkmc` CLI.
...
class ConstantsCfg:
    ...
    These defaults live in :mod:`ogkmc.core.constants`; exposing them here
...
class DiffusionCfg:
    ...
    All NEB defaults come from :mod:`ogkmc.core.constants` (``NEB_*``).
    """
    enabled:                   bool   = False
...
class BondCfg:
    ...
    1. Derives :class:`~ogkmc.sites.bond.BondReactionTemplate`'s from
       every reactant SMILES via
       :func:`~ogkmc.reactions.templates.derive_bond_templates`.
    ...
    3. Calls :func:`~ogkmc.sites.bond.find_bond_sites` to enumerate
       :class:`BondReactionSite` iso-classes on the live graph.
    4. Bootstraps :func:`~ogkmc.kmc.expansion.initialise_bond_registry`
...
class FreeEnergyCfg:
    ...
    *T* and a fixed 1-bar standard state) and for every successful adsorbate
    relaxation in :func:`ogkmc.sites.stability.adsorption.check_site_stability`

...
def _ogkmc_source_digest() -> str | None:
    """Hash installed OGKMC Python sources to detect an in-place code change."""
    package_file = getattr(ogkmc_package, "__file__", None)
    if not package_file:
        return None
...
def scientific_config_payload(cfg: Any) -> dict[str, Any]:
    ...
    payload["_software"] = {
        "ogkmc_version": ogkmc_package.__version__,
        "ogkmc_source_sha256": _ogkmc_source_digest(),

...
so OGKMC's readers can restore them exactly.
"""
...
STRING_ARRAYS_KEY = "ogkmc_extxyz_string_arrays"
...
def restore_string_arrays(atoms: Atoms) -> Atoms:
    ...
    if not isinstance(metadata, str) or not metadata.startswith("v1:"):
        raise ValueError("unsupported OGKMC EXTXYZ string-array metadata")
    encoded = metadata[3:]
...
def read_atoms(*args, **kwargs):
    """Read any ASE format, restoring OGKMC's escaped EXTXYZ string arrays."""

"""
ogkmc.sites.stability.adsorption
=============================
...
1. Call :func:`check_adsorbate_site_lateral` for any specific member of an
   :class:`~ogkmc.sites.adsorbate.AdsorbateSite`.
...
3. The ego-graph is compared by isomorphism to every
   :class:`~ogkmc.sites.adsorbate.AdsorbateSiteLateral` already stored
   on the parent :class:`~ogkmc.sites.adsorbate.AdsorbateSite`.
...
5. The matching (or newly created)
   :class:`~ogkmc.sites.adsorbate.AdsorbateSiteLateral` is returned.
...
:func:`~ogkmc.structure.optimise_structure`.  After each relaxation the
...
Energies are stored on the :class:`~ogkmc.sites.adsorbate.AdsorbateSiteLateral`
...
from ogkmc.core.constants import NL_MULT_DEFAULT, LATERAL_SHELLS_DEFAULT
...
def check_adsorbate_site_lateral(
    ...
    *,
    ...
    against every :class:`~ogkmc.sites.adsorbate.AdsorbateSiteLateral`
    ...
    G : nx.Graph
        ...
        :func:`~ogkmc.core.graph.build_graph` after
        :func:`~ogkmc.sites.adsorbate.find_adsorbate_sites` and with
    ...
    n_shells : int | None
        ...
        :data:`~ogkmc.core.constants.LATERAL_SHELLS_DEFAULT` (``0``), meaning
...
def _expand_to_full_placement(G: nx.Graph, seed_node_ids: set[int]) -> set[int]:
    ...
    :func:`~ogkmc.sites.adsorbate._materialise_adsorbate_nodes` so
...
def _bond_set(
    ...
    *,
    ...
    Uses the same ``natural_cutoffs`` scheme as :func:`~ogkmc.core.graph.build_graph`
...
def check_site_stability(
    ...
    * **unoccupied** — same slab + lateral neighbours, site absent.
    ...
    :func:`~ogkmc.structure.optimise_structure`.  Before and after
    ...
    nl_mult : float
        ...
        Default :data:`~ogkmc.core.constants.NL_MULT_DEFAULT`.
    verbose : bool
    ...
    """
    from ogkmc.structure import (  # local import avoids circular

"""
ogkmc.sites.adsorbate
============================
N-atom adsorbate site enumeration on a surface graph, built cleanly on top of
:mod:`ogkmc.sites.anchors` and :mod:`ogkmc.species.reactant`.

Where :func:`~ogkmc.sites.anchors.find_anchor_sites` finds all single-atom
adsorption *anchor* sites for one element, this module finds all geometrically
feasible placements of an arbitrary :class:`~ogkmc.species.reactant.Reactant`
...
3. **Backtracking chain placement.**  For every orbit-canonicalised non-empty
   ...
   * The first anchor is placed at every raw anchor-node position of its
     ...
     :func:`~ogkmc.sites.anchors.find_anchor_sites`).
...
from ogkmc.core.atom_metadata import (
...
# ---------------------------------------------------------------------------
# Tuneable defaults — single source of truth in :mod:`ogkmc.core.constants`.
...
class AdsorbateSiteLateral:
    ...
    :func:`ogkmc.sites.stability.adsorption.check_adsorbate_site_lateral` — one
    ...
    ego_graph : nx.Graph | None
        ...
        :func:`ogkmc.sites.stability.adsorption._build_lateral_ego_graph`.
    ...
    #: set by :func:`ogkmc.sites.stability.adsorption.check_site_stability`.
    ...
    #: ``None`` until :func:`ogkmc.sites.stability.adsorption.check_site_stability`
    #: has been run successfully.
    stable            : bool | None  = None
    #: Relaxed ASE :class:`~ase.Atoms` snapshot of the **occupied** state used
    #: in :func:`ogkmc.sites.stability.adsorption.check_site_stability` (i.e. the
    #: structure that produced ``energy_occupied``).  Persisted by
    #: :class:`ogkmc.io.persistence.ReactionWriter` as ``occupied.extxyz``
...
class AdsorbateSite:
    ...
    #: :func:`ogkmc.sites.stability.adsorption.check_adsorbate_site_lateral`).
    ...
    #: lazily by :func:`ogkmc.sites.identity.site_identifier` after graph
...
def _ensure_anchor_sites(
    ...
    *,
    ...
    """Lazily run :func:`~ogkmc.sites.anchors.find_anchor_sites` if needed."""
...
def _all_raw_sites_with_positions(
    ...
    :func:`~ogkmc.sites.anchors.find_anchor_sites` — one per raw clique,
...
def _build_pruning_atoms(
    ...
    *,
    ...
    array that mirrors :mod:`ogkmc.core.graph` conventions (0=bulk, 1=surface,
    2=adsorbate) so that :func:`ogkmc.core.graph.build_graph` can be called
...
def _intended_adsorbate_edges(
    ...
    * Anchor bonds: for every reactant atom *i* with
    ...
    edges actually produced by :func:`ogkmc.core.graph.build_graph` on the
...
def _geometry_connectivity_mismatch(
    ...
    *,
    ...
    used by :func:`ogkmc.core.graph.build_graph`, including its 0.3 Å
...
def prune_unstable_adsorbate_sites(
    ...
    *,
    ...
    1. Build a bare slab + adsorbate :class:`~ase.Atoms` from the
       ...
       ``surface == 2`` so :func:`ogkmc.core.graph.build_graph` can be called
       on it directly.
    ...
    3. Starting from that constrained minimum, run the existing full ML
       relaxation via :func:`~ogkmc.structure.optimise_structure`.  The
    ...
    ----------
    G : nx.Graph
        Surface graph from :func:`~ogkmc.core.graph.build_graph`.
    adsorbate_sites : list[AdsorbateSite]
    ...
    fmax : float
        ...
        relaxed stages.  Default :data:`~ogkmc.core.constants.PRUNE_FMAX`.
    max_steps : int
        Maximum optimizer steps for each of the two stages.  Default
        :data:`~ogkmc.core.constants.PRUNE_MAX_STEPS`.
    nl_mult : float
        Neighbour-list cutoff multiplier handed to
        :func:`ogkmc.core.graph.build_graph` for the connectivity comparison.
        Default :data:`~ogkmc.core.constants.NL_MULT_DEFAULT` — matches the
    ...
    kabsch_max_mappings : int
        ...
        :data:`~ogkmc.core.constants.KABSCH_MAX_MAPPINGS`.
    ...
    """
    from ogkmc.structure import (  # avoid circular at module level
        StructureOptimisationError,
        optimise_structure,
    )
    from ogkmc.core.graph import build_graph
    from ogkmc.io.atoms import copy_atoms_with_results
    from ogkmc.io.persistence import write_invalid_adsorption_diagnostic
...
def find_adsorbate_sites(
    ...
    *,
    ...
    ----------
    G : nx.Graph
        Surface graph from :func:`ogkmc.core.graph.build_graph`.  Anchor sites
        for every anchor element are lazily computed if not already on *G*.
    reactant : :class:`ogkmc.species.reactant.Reactant`
    ...
    co_factor, opt_factor, repulsion_weight, repulsion_cutoff :
        Forwarded to :func:`~ogkmc.sites.anchors.find_anchor_sites` if
    ...
    hull_tolerance : float
        ...
        :data:`~ogkmc.core.constants.HULL_TOL`.
    kabsch_max_mappings : int
    ...
    nl_mult : float
        ...
        :data:`~ogkmc.core.constants.NL_MULT_DEFAULT`.
    ...
    prune_fmax : float
        ...
        pruning stages.
        Default :data:`~ogkmc.core.constants.PRUNE_FMAX` (0.05).
    prune_max_steps : int
        Maximum optimizer steps for each pruning stage.
        Default :data:`~ogkmc.core.constants.PRUNE_MAX_STEPS` (500).
    diagnostics_dir : str | None

...
between materialised :class:`~ogkmc.sites.adsorbate.AdsorbateSite`'s.
...
:mod:`ogkmc.sites.adsorbate` / :mod:`ogkmc.sites.stability.adsorption`
and :mod:`ogkmc.sites.diffusion` / :mod:`ogkmc.sites.stability.diffusion`:
...
* :mod:`ogkmc.sites.stability.bond` — on-the-fly lateral-environment
...
Templates are derived from :mod:`ogkmc.species.bond_chemistry`:
...
from ogkmc.core.atom_metadata import apply_atom_metadata, atom_metadata_key
from ogkmc.sites.adsorbate import (
    AdsorbateSite,
)
from ogkmc.sites.identity import SiteId, member_identifier, site_identifier
from ogkmc.sites.diffusion import _member_clique_union, _reactant_orbit_label
from ogkmc.sites.stability.adsorption import _surface_bfs_shells
...
from ogkmc.utils.optimizers import DEFAULT_OPTIMIZER
...
class BondReactionTemplate:
    ...
    bond_type : str
        ...
        :func:`ogkmc.species.bond_chemistry.combine_fragments`.
...
class BondReactionLateral:
    ...
    Mirrors :class:`ogkmc.sites.diffusion.DiffusionLateral` but for a
    three-state reversible reaction.  Energies / atoms / TS are filled in
    lazily during the KMC loop by :mod:`ogkmc.sites.stability.bond`.
    ...
    ego_graph : nx.Graph | None
        ...
        on-the-fly lateral classifier in :mod:`ogkmc.sites.stability.bond`).
    n_shells : int
    ...
    atoms_ab, atoms_c, atoms_ts : Atoms | None
        ...
        :class:`ogkmc.io.persistence.ReactionWriter`.
...
class BondReactionSite:
    ...
    lateral_classes : list[BondReactionLateral]
        Lazily populated by
        :func:`ogkmc.sites.stability.bond.check_bond_site_lateral`.
    """
    template        : BondReactionTemplate
    iso_class       : int
    ...
    # populated by :func:`find_bond_sites`.  Used by :mod:`ogkmc.reactions.bond`
...
def derive_dissociation_templates(
    ...
    *,
    ...
    :func:`ogkmc.species.bond_chemistry.get_all_fragments` (with
    ...
    smiles, bond_types, include_ring_bonds, add_hydrogens
        Forwarded to :func:`ogkmc.species.bond_chemistry.get_all_fragments`
...
def derive_coupling_templates(
    ...
    *,
    ...
    :class:`~ogkmc.species.bond_chemistry.CombinedSpecies` returned by
    :func:`ogkmc.species.bond_chemistry.combine_fragments`.
...
def _build_triple_ego_graph(
    ...
    *,
    ...
    Generalisation of
    :func:`ogkmc.sites.diffusion._build_pair_ego_graph` to three
...
def _prune_one_per_adsorption_triple(
    ...
    *,
    ...
    Mirrors :func:`ogkmc.sites.diffusion._prune_one_per_adsorption_pair`.
    """
...
def find_bond_sites(
    ...
    *,
    ...
    adsorbate_sites : iterable of AdsorbateSite
        ...
        :func:`~ogkmc.sites.adsorbate.find_adsorbate_sites` output
    ...
    max_hops : int
        ...
        :data:`ogkmc.core.constants.BOND_MAX_HOPS`).
...
def prune_unstable_bond_sites(
    ...
    **after** this function — viability comes first.
    ...
    species_by_smiles : Mapping[str, Reactant]
        Map from canonical SMILES → :class:`~ogkmc.species.reactant.Reactant`.
        Used to look up chemical symbols and intramolecular bond
        topology.  Pass ``G.graph["bond_registry"]["species"]`` after
        :func:`ogkmc.kmc.expansion.initialise_bond_registry` has run.
    calculator
    ...
    frozen_indices, fmax, max_steps, nl_mult, verbose
        See :func:`ogkmc.sites.adsorbate.prune_unstable_adsorbate_sites`.
    debug_output_dir
    ...
    def _write_debug_endpoint(bond_iso, filename: str, atoms) -> None:
        if debug_dir is None:
            return
        try:
            from ogkmc.io.extxyz import write_extxyz as ase_write

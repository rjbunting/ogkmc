"""
ogkmc.sites.diffusion
============================
Enumerate diffusion (hop) site-pairs on top of materialised
:class:`~ogkmc.sites.adsorbate.AdsorbateSite`'s.
...
* a **surface-graph link** between their bonded surface cliques: the minimum
  ...
  most ``max_hops`` (default :data:`~ogkmc.core.constants.DIFFUSION_MAX_HOPS`
...
:func:`ogkmc.sites.adsorbate.prune_unstable_adsorbate_sites`, so every
endpoint is known stable.  Lateral-environment classification and the NEB
transition-state search are deferred to
:mod:`ogkmc.sites.stability.diffusion` and run lazily during the KMC loop —
...
from ogkmc.sites.identity import SiteId, site_identifier
...
class DiffusionLateral:
    ...
    Mirrors :class:`ogkmc.sites.adsorbate.AdsorbateSiteLateral` for
    the diffusion case.  Energies and relaxed atoms are filled in lazily by
    :func:`ogkmc.sites.stability.diffusion.check_diffusion_stability` once the
    ...
    ego_graph : nx.Graph | None
        ...
        :func:`ogkmc.sites.stability.diffusion.check_diffusion_site_lateral`.
    n_shells : int
...
def _member_clique_union(site: AdsorbateSite, m_idx: int) -> frozenset[int]:
    ...
    by :func:`~ogkmc.sites.adsorbate.find_adsorbate_sites`.
    """
...
def find_diffusion_sites(
    ...
    *,
    ...
    max_hops : int
        ...
        :data:`~ogkmc.core.constants.DIFFUSION_MAX_HOPS` (= 1).
    n_shells_pair : int

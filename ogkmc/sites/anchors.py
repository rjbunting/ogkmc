...
from ogkmc.core.atom_metadata import atom_metadata_key, physical_node_match
...
# Re-exported here so callers can import constants from this domain module.
...
def _mic_distances(
    ...
    return minimum_image_distances(p[np.newaxis, :] - ref, cell, pbc)
...
def _mic_unwrap(
    ...
    for n in nodes:
        ...
        if use_mic and cell_inv is not None:
            p = anchor + minimum_image_vectors(p - anchor, cell, pbc)
        out[int(n)] = p
    return out
...
def _kabsch_align_ego(
    ...
    tried (defaults to :data:`ogkmc.core.constants.KABSCH_MAX_MAPPINGS`) so that
...
_NODE_ID_CURSOR = "_ogkmc_next_node_id"

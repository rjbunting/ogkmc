"""Read-only readiness checks for configured OGKMC runs."""
...
def _checkpoint_readiness(cfg: RunConfig) -> tuple[dict[str, Any], list[str]]:
    from ogkmc.io.checkpoint import load_checkpoint
    from ogkmc.io.resume_contract import verify_resume_contract
...
def _exercise_calculator(
    ...
    *,
    ...
    calculator = None
    try:
        with tempfile.TemporaryDirectory(prefix="ogkmc-preflight-") as temporary:
...
def preflight_config(
    ...
    *,
    ...
    from ogkmc.io.config import ConfigError
    ...
    if owner is not None:
        errors.append(
            f"another OGKMC run is using output.dir {output_dir}: {owner}"
        )
...
def doctor_report(
    ...
    *,
    ...
    if sys.version_info < (3, 10):
        issues.append(
            "OGKMC requires Python >= 3.10, found "

...
2. **Find adsorbate sites for C** — so the geometric enumerator
   :func:`ogkmc.sites.bond.find_bond_sites` has placements to
   match against.
...
The actual list of enumerated :class:`~ogkmc.sites.bond.BondReactionSite`'s
...
* :func:`expand_bond_sites_after_event`   — convenience hook that takes
  a fired :class:`~ogkmc.reactions.bond.BondReaction` and triggers expansion
...
from ogkmc.sites.adsorbate import AdsorbateSite, find_adsorbate_sites
...
def initialise_bond_registry(
    ...
    *,
    ...
    :func:`ogkmc.sites.bond.find_bond_sites` so subsequent calls to
    ...
    expanded_smiles : iterable[str] | None
        ...
        :func:`~ogkmc.reactions.templates.derive_bond_templates`, not merely
...
def expand_bond_sites_for_new_species(
    ...
    *,
    ...
    2. Build a :class:`~ogkmc.species.reactant.Reactant` and find
       :class:`~ogkmc.sites.adsorbate.AdsorbateSite`'s for it.
    ...
    6. Run :func:`ogkmc.sites.bond.find_bond_sites` for the new
    ...
    calculator
        ASE calculator handed to :func:`ogkmc.species.reactant.build_reactant`
        and :func:`ogkmc.sites.adsorbate.find_adsorbate_sites`.
...
def expand_bond_sites_after_event(
    ...
    **expand_kwargs,
    ...
    :class:`~ogkmc.reactions.bond.BondReaction` (``kind == "bond"``), triggers

...
:class:`~ogkmc.sites.diffusion.DiffusionSite`'s.

Mirrors :mod:`ogkmc.reactions.adsorption` for adsorption / desorption.  For
...
3. **Lateral classification** — :func:`ogkmc.sites.stability.diffusion.
...
4. **NEB stability** — the first time a member maps to a new lateral class,
   :func:`ogkmc.sites.stability.diffusion.check_diffusion_stability` relaxes
...
5. **Energetics** for the firing direction::
   ...
   reaction folder by :class:`ogkmc.io.persistence.ReactionWriter`; the
   ``EA_MIN`` floor is only applied when computing the rate consumed by
   the KMC loop, exactly as documented on
   :data:`ogkmc.reactions.adsorption.EA_MIN`.
...
from ogkmc.sites.diffusion import DiffusionSite, DiffusionLateral

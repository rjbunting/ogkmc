...
from ogkmc.core.graph_state import set_bond_reaction_sites
from ogkmc.species.smiles import (
    canonical_smiles,
    reactant_atom_inventory_smiles,
)
from ogkmc.workflow.models import PreparedNetwork, RunIdentity, ThermoRuntime
...
def derive_configured_bond_templates(reactant_configs, reactants, bond_cfg):
    ...
    from ogkmc.reactions.templates import derive_bond_templates
...
class SpeciesNetworkBuilder:
    ...
    def _discover_diffusion(self, sites: list) -> list:
        ...
        if not settings.enabled:
            return []
        if self.verbose:
            print("[ogkmc]   diffusion enabled: enumerating hop permutations")
        ...
        if self.verbose:
            print(
                f"[ogkmc] Diffusion enabled: {len(result)} DiffusionSite "
                f"iso-class(es) across {len(by_smiles)} SMILES."
            )
        return result

    def _discover_bonds(self, reactants: list, sites: list) -> list:
        from ogkmc.kmc.expansion import initialise_bond_registry
        ...
        if self.verbose:
            print(
                f"[ogkmc] Bond reactions enabled: derived {len(templates)} "
        ...
        if self.verbose:
            print(
                f"[ogkmc] Bond reactions: {len(bond_sites)} "
    ...
    def _build_leaf_species(
        ...
        for canonical in leaves:
            if self.verbose:
                print(
                    f"[ogkmc]   leaf species: {canonical!r} — building "
            ...
            if self.verbose:
                ...
                print(
                    f"[ogkmc]   leaf species {canonical!r}: {n_iso} stable "

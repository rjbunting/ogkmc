...
from ogkmc.io.calculation_cache import initialise_calculation_database
from ogkmc.io.checkpoint import CheckpointWriter, load_checkpoint
from ogkmc.io.event_log import reconcile_event_log
from ogkmc.io.persistence import ReactionWriter
from ogkmc.io.resume_contract import make_resume_contract, verify_resume_contract
from ogkmc.io.summary import ReactionSummary
...
def configured_run_lock(output_dir: str | Path):
    ...
    with _ACTIVE_RUN_LOCKS_GUARD:
        if lock_key in _ACTIVE_RUN_LOCKS:
            raise RunLockError(
                f"another OGKMC run in this process already owns {directory}"
            )
    ...
    try:
        ...
        try:
            ...
            raise RunLockError(
                f"another OGKMC process is already using output directory "
                f"{directory}.{detail}"
            ) from exc
        acquired = True
        handle.seek(0)

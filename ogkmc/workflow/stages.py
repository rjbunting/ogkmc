...
from ogkmc.core.graph_state import set_frozen_indices, set_run_id
...
def prepare_calculator(cfg, *, verbose: bool = False) -> PreparedCalculator:
    ...
    if calculator is None:
        ...
        print(f"[ogkmc]   calculator ready: {type(calculator).__name__}")
    ...
    *,
    ...
    if structure_cfg.kind == "file":
        ...
        if verbose:
            cell = atoms.get_cell()
            print(
                f"[ogkmc]   structure loaded successfully: {len(atoms)} atoms, "
            ...
            if frozen_indices:
                print(
                    f"[ogkmc]   frozen region: {len(frozen_indices)} "
                    "atom(s) fixed"
                )
            else:
                print("[ogkmc]   frozen region: no fixed atoms")
        return PreparedStructure(
            atoms=atoms,
    ...
    from ogkmc.structure import build_nanoparticle, build_surface
    ...
    if verbose:
        cell = atoms.get_cell()
        print(
            f"[ogkmc]   structure built successfully: {len(atoms)} atoms, "
        ...
        if frozen_indices:
            print(
                "[ogkmc]   frozen region: requested bottom "
...
def prepare_material_graph(
    ...
    *,
    ...
    from ogkmc.structure import align_periodic_slab_frame, find_surface_atoms
    from ogkmc.structure.surface import _pbc_connectivity_axes
        ...
        if structure_settings.kind == "file":
            ...
            if int(np.count_nonzero(connectivity_axes)) == 2:
                ...
                if verbose and frame_metadata["rotation_applied"]:
                    print(
                        "[ogkmc]   file-backed slab rigidly aligned: "
                        "surface normal → +z"
                    )
    ...
    if verbose and surface_result is not None and atoms is not None:
        print(
            "[ogkmc]   surface classification successful: "
            f"{len(surface_result.indices)}/{len(atoms)} surface atom(s), "
            f"method={surface_result.method}"
        )
        print(
            f"[ogkmc]   graph ready: {graph.number_of_nodes()} node(s), "
            f"{graph.number_of_edges()} edge(s)"
        )
    elif verbose:
        print(
            f"[ogkmc]   checkpoint graph restored: "
    ...
    *,
    ...
    if identity.resume_state is not None:
        ...
        for reactant_cfg in cfg.reactants:
            if verbose:
                print(
                    f"[ogkmc]   reactant {reactant_cfg.smiles!r}: "
            ...
            if verbose:
                ...
                print(
                    f"[ogkmc]   reactant {reactant.smiles!r}: ready, "
...
def prepare_adsorbate_sites(
    ...
    *,
    ...
    from ogkmc.sites.adsorbate import find_adsorbate_sites
    ...
    for reactant in reactants:
        if verbose:
            if settings.prune_stable_only and calculator_resource is not None:
                print(
                    f"[ogkmc]   {reactant.smiles!r}: searching placements; "
        ...
        if verbose:
            ...
            print(
                f"[ogkmc]   {reactant.smiles!r}: {n_iso} stable adsorbate "

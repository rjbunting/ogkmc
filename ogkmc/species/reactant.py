...
4. Build a :class:`networkx.Graph` via :func:`~ogkmc.core.graph.build_graph`
...
from ogkmc.core.atom_metadata import node_mass, physical_node_match
...
class Reactant:
    ...
    graph : nx.Graph
        Connectivity graph built by :func:`~ogkmc.core.graph.build_graph`.
    ...
    energy : float
        ...
        ``E_gas`` in :func:`ogkmc.sites.adsorbate.optimise_unique_configurations`.
    ...
    #: adsorption rate in :func:`ogkmc.reactions.adsorption._energetics_cached`
...
def _smiles_to_atoms(
    ...
    *,
    ...
    random_seed : int
        ...
        fallback.  Default :data:`~ogkmc.core.constants.RANDOM_SEED`.
...
def find_anchor_atoms(
    ...
    *,
    ...
    :func:`ogkmc.sites.adsorbate.enumerate_configurations`.  An atom is
...
def build_reactant(
    ...
    *,
    ...
    nl_mult : float
        Neighbour-list multiplier passed to :func:`~ogkmc.core.graph.build_graph`.
        Default ``NL_MULT_DEFAULT`` (currently 0.90).
    random_seed : int
        Seed used for RDKit conformer embedding.  Default
        :data:`~ogkmc.core.constants.RANDOM_SEED`.
    hull_tol : float
    ...
    if (
        ...
        from ogkmc.thermo.free_energy import compute_gas_thermo

...
* ``type``           – one of ``"bulk"``, ``"surface"``, ``"adsorbate"``,
  ...
  :func:`ogkmc.sites.anchors.find_anchor_sites` (one node per
  raw site clique).
...
``mult=ogkmc.core.constants.NL_MULT_DEFAULT``).
...
from ogkmc.core.constants import NEIGHBORLIST_SKIN, NL_MULT_DEFAULT
from ogkmc.core.atom_metadata import atom_metadata
from ogkmc.core.pbc import graph_pbc_for_atoms
...
def build_graph(
    ...
    *,
    ...
    :func:`ogkmc.structure.tag_surface_atoms` or by calling
    :func:`ogkmc.structure.find_surface_atoms` with ``tag_atoms=True``.
    ...
    nl_mult : float
        ...
        :data:`ogkmc.core.constants.NL_MULT_DEFAULT` (currently ``1.0``).
    ...
    if "_ogkmc_surface_side" in atoms.info:
        G.graph["surface_side"] = atoms.info["_ogkmc_surface_side"]

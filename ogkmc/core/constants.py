...
Every other module imports its defaults from here::
    ...
    from ogkmc.core.constants import CO_FACTOR, OPT_FACTOR, ...
...
#: (alloy substitution in :func:`ogkmc.structure._apply_composition`,
#: RDKit ETKDGv3 conformer embedding in
#: :func:`ogkmc.species.reactant._smiles_to_atoms`, …).  Centralized so a single
...
#: :func:`ogkmc.core.graph.build_graph`,
#: :func:`ogkmc.structure.find_surface_atoms`
#: and :func:`ogkmc.species.reactant.build_reactant`.  A single value is used
...
#: :func:`ogkmc.sites.anchors._build_co_bond_graph`: two surface atoms can
...
#: position from :func:`ogkmc.sites.anchors._optimise_position` directly as
...
#: :func:`ogkmc.sites.anchors._optimise_position`.  Only atoms
...
#: :func:`ogkmc.sites.anchors._reduce_by_isomorphism`.  ``1``
#: distinguishes fcc vs hcp hollows on Cu(111); ``0`` collapses them.
N_SHELLS_DEFAULT: int = 1

#: Alias matching the per-module name in :mod:`ogkmc.sites.anchors`.
...
#: :func:`ogkmc.sites.adsorbate._suggested_n_shells` to pick a default
...
#: :func:`ogkmc.sites.adsorbate.find_adsorbate_sites`.
MAX_PAIR_SHELLS: int = 10

#: Default tolerance (Å) when matching surface anchor-pair distances to
#: intramolecular distances in :func:`ogkmc.sites.adsorbate.find_adsorbate_sites`.
...
#: :func:`ogkmc.sites.adsorbate.prune_unstable_adsorbate_sites` to
...
#: :func:`ogkmc.sites.adsorbate.prune_unstable_adsorbate_sites`.
...
#: spurious site in :func:`ogkmc.sites.anchors._enumerate_cliques`.
HULL_TOL: float = -0.2
...
#: "exposed" in :func:`ogkmc.structure.find_surface_atoms_raycasting`.
#: Used by both the dispatcher (:func:`ogkmc.structure.find_surface_atoms`)
#: and the reverse-ray-casting layer-freezing helper
#: (:func:`ogkmc.structure._get_bottom_layer_indices`) so the same
#: threshold is applied everywhere.
RAYCAST_COVERAGE_THRESHOLD: float = 0.7

#: Default ray-disc resolution (per axis) in
#: :func:`ogkmc.structure.find_surface_atoms_raycasting`.  ``n_disc_sample=10``
...
#: :func:`ogkmc.sites.anchors._kabsch_align_ego` when searching for the
...
# ---------------------------------------------------------------------------
# Persistence / CLI / output (consumed by ogkmc.io + ogkmc.cli)
# ---------------------------------------------------------------------------

#: Schema version stamped onto every reaction event and summary JSON document
#: written by :mod:`ogkmc.io.persistence`.  Bump on any breaking schema change.
PERSISTENCE_SCHEMA_VERSION: str = "2"

#: Default config-file schema version expected by :mod:`ogkmc.io.config`.
CONFIG_SCHEMA_VERSION: str = "1"

#: Default cadence for :class:`ogkmc.io.trajectory.TrajectoryWriter` — write
#: an ASE ``.traj`` frame every N executed KMC steps.  ``0`` disables.
TRAJ_DUMP_EVERY: int = 10

#: Default BFS depth used by
#: :func:`ogkmc.sites.stability.adsorption.check_adsorbate_site_lateral` when
...
#: etc.  The :mod:`ogkmc.kmc.engine` incremental trigger radius is
#: automatically derived from this value.
LATERAL_SHELLS_DEFAULT: int = 0

#: Default basenames written by :func:`ogkmc.cli.pipeline.run_from_config`.
...
DEFAULT_OUTPUT_DIR:   str = "ogkmc_run"
...
#: persisted reaction event.  See :class:`ogkmc.io.records.ReactionRecord`.
...
#: :func:`ogkmc.sites.diffusion.find_diffusion_sites`).  ``0`` requires
...
#: :func:`ogkmc.sites.stability.diffusion.check_diffusion_stability`.
NEB_N_IMAGES: int = 10
...
#: When ``True`` (default), :func:`ogkmc.sites.diffusion.find_diffusion_sites`
#: keeps only **one** :class:`~ogkmc.sites.diffusion.DiffusionSite` per
...
#: :class:`ogkmc.io.persistence.ReactionWriter`.  See
#: :func:`ogkmc.io.persistence._diffusion_folder_name`.
...
#: :func:`ogkmc.sites.bond.find_bond_sites`.
BOND_MAX_HOPS: int = 0

#: When ``True`` (default), :func:`ogkmc.sites.bond.find_bond_sites`
#: keeps only **one** :class:`~ogkmc.sites.bond.BondReactionSite` per
...
#: When ``True`` (default), :func:`ogkmc.sites.bond.prune_unstable_bond_sites`
...
#: :func:`ogkmc.sites.bond.find_bond_sites` for triple iso-class
...
#: :class:`ogkmc.io.persistence.ReactionWriter` (when enabled).

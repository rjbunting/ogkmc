...
def _optimize_site_position(
    ...
    *Repulsion term* – soft repulsion from non-bonded surface atoms,
    ...
    This is particularly important for top and bridge sites where there is
    lateral freedom::

        repulsion = sum_j 1 / |p - pos_j|_MIC^2   (j not in clique)

    All distances use the minimum-image convention (MIC) with the periodic
    boundary conditions stored in ``G.graph["pbc"]``.
    ...
    # ------------------------------------------------------------------ #
    # Geometric starting point (MIC-aware centroid)                     #
    # ------------------------------------------------------------------ #
    ref = bonded_pos[0]
    if use_mic:
    ...
    # Height above the surface plane:
    #   h_i = sqrt(max(0, ideal_i^2 - lateral_i^2))
    # where lateral_i = in-plane distance from centroid to atom i (MIC).
    lateral_dists = np.linalg.norm(mic_rel[:, :2] - mic_rel[:, :2].mean(axis=0),
                                   axis=1)
    ...
    # Enforce z >= surface level so the adsorbate cannot slip below the
    # bonded atoms (prevents the optimizer exploiting the bond-sphere
    # intersection below the surface).
...
def reduce_sites_by_isomorphism(
    ...
    *,
    ...
    Shell depth controls discrimination:
      n_shells=1  -- direct bonding neighbors only.  Coarse: fcc and hcp
                     hollows on Cu(111) look identical.
      n_shells=2  -- adds the second shell.  Fine: fcc vs hcp hollows are
                     now distinguished by the subsurface atom beneath the fcc
                     site.
      n_shells=3+ -- deeper shells; useful for step-edge or defect sites.

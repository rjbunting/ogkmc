"""
autokmc.find_bond_sites
=======================
Enumerate **bond-changing surface reactions**

    A + B  ⇌  C

between materialised :class:`~autokmc.find_adsorbate_sites.AdsorbateSite`'s.
...
:mod:`autokmc.find_adsorbate_sites` / :mod:`autokmc.check_adsorbate_sites`
and :mod:`autokmc.find_diffusion_sites` / :mod:`autokmc.check_diffusion_sites`:
...
* :mod:`autokmc.check_bond_sites` — on-the-fly lateral-environment
...
Templates are derived from :mod:`autokmc.molecular_bond_changing`:
...
from .find_diffusion_sites import _member_clique_union
from .check_adsorbate_sites import _surface_bfs_shells
...
class BondReactionTemplate:
    ...
    bond_type : str
        ...
        :func:`autokmc.molecular_bond_changing.combine_fragments`.
...
class BondReactionLateral:
    ...
    Mirrors :class:`autokmc.find_diffusion_sites.DiffusionLateral` but for a
    three-state reversible reaction.  Energies / atoms / TS are filled in
    lazily during the KMC loop by :mod:`autokmc.check_bond_sites`.
    ...
    ego_graph : nx.Graph | None
        ...
        on-the-fly lateral classifier in :mod:`autokmc.check_bond_sites`).
    n_shells : int
...
class BondReactionSite:
    ...
    lateral_classes : list[BondReactionLateral]
        Lazily populated by
        :func:`autokmc.check_bond_sites.check_bond_site_lateral`.
    """
    template        : BondReactionTemplate
    iso_class       : int
    ...
    # populated by :func:`find_bond_sites`.  Used by :mod:`autokmc.kmc_bond`
...
def derive_dissociation_templates(
    ...
    *,
    ...
    :func:`autokmc.molecular_bond_changing.get_all_fragments` (with
    ...
    smiles, bond_types, include_ring_bonds, add_hydrogens
        Forwarded to :func:`autokmc.molecular_bond_changing.get_all_fragments`
        (the latter receives ``strip_dummies=True``).
    """
    from .molecular_bond_changing import get_all_fragments
...
def derive_coupling_templates(
    ...
    *,
    ...
    :class:`~autokmc.molecular_bond_changing.CombinedSpecies` returned by
    :func:`autokmc.molecular_bond_changing.combine_fragments`.
    ...
    include_hetero : bool
        ...
        Default ``True``.
    """
    from .molecular_bond_changing import combine_fragments
...
def _placement_key(site: AdsorbateSite, m_idx: int) -> tuple[int, int, int]:
    """Canonical sortable key for one (site, m_idx) placement."""
    return (id(site), int(site.iso_class), int(m_idx))
...
def _build_triple_ego_graph(
    ...
    *,
    ...
    Generalisation of
    :func:`autokmc.find_diffusion_sites._build_pair_ego_graph` to three
...
def _prune_one_per_adsorption_triple(
    ...
    *,
    ...
    For every unordered triple ``(frozenset({iso_a, iso_b}), iso_c)`` that
    appears in *bond_sites*, retain only the iso-class whose ``ego_graph``
    has the fewest ``nodes + edges`` (most direct / closest reaction).
    Mirrors :func:`autokmc.find_diffusion_sites._prune_one_per_adsorption_pair`.
    """
    ...
    for brs in bond_sites:
        ...
        key = (
            frozenset({int(sa.iso_class), int(sb.iso_class)}),
            int(sc.iso_class),
        )
...
def find_bond_sites(
    ...
    *,
    ...
    adsorbate_sites : iterable of AdsorbateSite
        ...
        :func:`~autokmc.find_adsorbate_sites.find_adsorbate_sites` output
    ...
    max_hops : int
        ...
        :data:`autokmc.constants.BOND_MAX_HOPS`).
    surface_apsp_cutoff : int
        Cutoff handed to
        :func:`autokmc.find_adsorbate_sites._get_surface_apsp` for the
    ...
    bond_clique_to_members: dict = G.graph.setdefault(
        "bond_clique_to_members", {}
    )
    bond_surface_to_members: dict = G.graph.setdefault(
    ...
    for new_idx, brs in enumerate(out):
        ...
        for m_idx, (cliques_a, cliques_b, cliques_c) in enumerate(
            ...
            for clq in (*cliques_a, *cliques_b, *cliques_c):
                bond_clique_to_members.setdefault(clq, []).append(
                    (brs, m_idx)
                )
                for surf_id in clq:
...
def prune_unstable_bond_sites(
    ...
    **after** this function — viability comes first.
    ...
    species_by_smiles : Mapping[str, Reactant]
        Map from canonical SMILES → :class:`~autokmc.reactants.Reactant`.
        Used to look up chemical symbols and intramolecular bond
        topology.  Pass ``G.graph["bond_registry"]["species"]`` after
        :func:`autokmc.grow_bond_sites.initialise_bond_registry` has run.
    calculator
    ...
    frozen_indices, fmax, max_steps, nl_mult, verbose
        See :func:`autokmc.find_adsorbate_sites.prune_unstable_adsorbate_sites`.
    ...
    # Renumber and rebuild the clique reverse-index from scratch so it is
    # consistent with the surviving set.
    G.graph["bond_clique_to_members"] = {}
    G.graph["bond_surface_node_to_members"] = {}
    rebuilt: dict = G.graph["bond_clique_to_members"]
    ...
    for new_idx, brs in enumerate(survivors):
        ...
        for m_idx, (cliques_a, cliques_b, cliques_c) in enumerate(
            ...
            for clq in (*cliques_a, *cliques_b, *cliques_c):
                rebuilt.setdefault(clq, []).append((brs, m_idx))
                for surf_id in clq:
                    rebuilt_surf.setdefault(int(surf_id), []).append(
                        (brs, m_idx)
                    )

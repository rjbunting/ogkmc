...
* ``autokmc preflight CONFIG``       — check run readiness without writing.
* ``autokmc doctor [CONFIG]``        — inspect runtime/package readiness.
* ``autokmc analyze RUN_DIR``        — analyze products and mechanisms.
* ``autokmc report RUN_DIR``         — write Markdown and HTML run reports.
...
def load_config(path: str | Path) -> Any:
    from autokmc.io.config import load_config as implementation

    return implementation(path)


def preflight_config(cfg: Any, **kwargs: Any) -> dict[str, Any]:
    from autokmc.cli.diagnostics import preflight_config as implementation

    return implementation(cfg, **kwargs)


def doctor_report(cfg: Any = None, **kwargs: Any) -> dict[str, Any]:
    from autokmc.cli.diagnostics import doctor_report as implementation
...
def analyze_run(run_dir: str | Path, **kwargs: Any) -> dict[str, Any]:
    from autokmc.analysis.products import analyze_run as implementation

    return implementation(run_dir, **kwargs)


def generate_run_report(run_dir: str | Path, **kwargs: Any) -> dict[str, Any]:
    from autokmc.analysis.run_report import generate_run_report as implementation
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_report.add_argument("run_dir", help="completed AutoKMC run directory")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print("autokmc: interrupted", file=sys.stderr)
        return 130
    except Exception as exc:
        if args.debug:
            raise
        if _is_expected_cli_error(exc):
            print(f"autokmc: error: {exc}", file=sys.stderr)
            return 2
        message = str(exc).strip() or "no additional detail"
        print(
            f"autokmc: error: {type(exc).__name__}: {message}",
            file=sys.stderr,
        )
        print(

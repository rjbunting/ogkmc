"""
autokmc.kmc_diffusion
=====================
Build KMC diffusion (hop) reactions for a graph of materialised
:class:`~autokmc.find_diffusion_sites.DiffusionSite`'s.

Mirrors :mod:`autokmc.kmc_adsorption` for adsorption / desorption.  For
...
3. **Lateral classification** — :func:`autokmc.check_diffusion_sites.
...
4. **NEB stability** — the first time a member maps to a new lateral class,
   :func:`autokmc.check_diffusion_sites.check_diffusion_stability` relaxes
...
from autokmc.find_diffusion_sites import DiffusionSite, DiffusionLateral
...
def _target_blocked(
    ...
    if member_cliques is None or occupied_by_clique is None:
        return False  # safe default — fall through to ML stability check

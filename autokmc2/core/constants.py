...
Every other module imports its defaults from here::
    ...
    from autokmc2.core.constants import CO_FACTOR, OPT_FACTOR, ...
...
#: (alloy substitution in :func:`autokmc2.structure._apply_composition`,
#: RDKit ETKDGv3 conformer embedding in
#: :func:`autokmc2.species.reactant._smiles_to_atoms`, …).  Centralized so a single
...
#: :func:`autokmc2.core.graph.build_graph`,
#: :func:`autokmc2.structure.find_surface_atoms`
#: and :func:`autokmc2.species.reactant.build_reactant`.  A single value is used
...
#: :func:`autokmc2.sites.anchors._build_co_bond_graph`: two surface atoms can
...
#: :func:`autokmc2.sites.anchors._optimise_position` (already placed at
...
#: :func:`autokmc2.sites.anchors._optimise_position`.  Only atoms
...
#: :func:`autokmc2.sites.anchors._reduce_by_isomorphism`.  ``1``
#: distinguishes fcc vs hcp hollows on Cu(111); ``0`` collapses them.
N_SHELLS_DEFAULT: int = 1

#: Alias matching the per-module name in :mod:`autokmc2.sites.anchors`.
...
#: :func:`autokmc2.sites.adsorbate._suggested_n_shells` to pick a default
...
#: :func:`autokmc2.sites.adsorbate.find_adsorbate_sites`.
MAX_PAIR_SHELLS: int = 10

#: Default tolerance (Å) when matching surface anchor-pair distances to
#: intramolecular distances in :func:`autokmc2.sites.adsorbate.find_adsorbate_sites`.
...
#: :func:`autokmc2.sites.adsorbate.prune_unstable_adsorbate_sites` to
...
#: :func:`autokmc2.sites.adsorbate.prune_unstable_adsorbate_sites`.
...
#: spurious site in :func:`autokmc2.sites.anchors._enumerate_cliques`.
HULL_TOL: float = -0.2
...
#: "exposed" in :func:`autokmc2.structure.find_surface_atoms_raycasting`.
#: Used by both the dispatcher (:func:`autokmc2.structure.find_surface_atoms`)
#: and the reverse-ray-casting layer-freezing helper
#: (:func:`autokmc2.structure._get_bottom_layer_indices`) so the same
#: threshold is applied everywhere.
RAYCAST_COVERAGE_THRESHOLD: float = 0.7

#: Default ray-disc resolution (per axis) in
#: :func:`autokmc2.structure.find_surface_atoms_raycasting`.  ``n_disc_sample=10``
...
#: :func:`autokmc2.sites.anchors._kabsch_align_ego` when searching for the
...
# ---------------------------------------------------------------------------
# Persistence / CLI / output (consumed by autokmc2.io + autokmc2.cli)
# ---------------------------------------------------------------------------

#: Schema version stamped onto every reaction event and summary JSON document
#: written by :mod:`autokmc2.io.persistence`.  Bump on any breaking schema change.
PERSISTENCE_SCHEMA_VERSION: str = "1"

#: Default config-file schema version expected by :mod:`autokmc2.io.config`.
CONFIG_SCHEMA_VERSION: str = "1"

#: Default cadence for :class:`autokmc2.io.trajectory.TrajectoryWriter` — write
#: an ASE ``.traj`` frame every N executed KMC steps.  ``0`` disables.
TRAJ_DUMP_EVERY: int = 10

#: Default BFS depth used by
#: :func:`autokmc2.sites.stability.adsorption.check_adsorbate_site_lateral` when
...
#: etc.  The :mod:`autokmc2.kmc.engine` incremental trigger radius is
#: automatically derived from this value.
LATERAL_SHELLS_DEFAULT: int = 0

#: Default basenames written by :func:`autokmc2.cli.main.run_from_config`.
...
#: persisted reaction event.  See :class:`autokmc2.io.records.ReactionRecord`.
...
#: :func:`autokmc2.sites.diffusion.find_diffusion_sites`).  ``0`` requires
...
#: :func:`autokmc2.sites.stability.diffusion.check_diffusion_stability`.
NEB_N_IMAGES: int = 10
...
#: When ``True`` (default), :func:`autokmc2.sites.diffusion.find_diffusion_sites`
#: keeps only **one** :class:`~autokmc2.sites.diffusion.DiffusionSite` per
...
#: :class:`autokmc2.io.persistence.ReactionWriter`.  See
#: :func:`autokmc2.io.persistence._diffusion_folder_name`.
...
#: :func:`autokmc2.sites.bond.find_bond_sites`.
BOND_MAX_HOPS: int = 0

#: When ``True`` (default), :func:`autokmc2.sites.bond.find_bond_sites`
#: keeps only **one** :class:`~autokmc2.sites.bond.BondReactionSite` per
...
#: When ``True`` (default), :func:`autokmc2.sites.bond.prune_unstable_bond_sites`
...
#: :func:`autokmc2.sites.bond.find_bond_sites` for triple iso-class
#: deduplication / pruning.
BOND_PAIR_N_SHELLS: int = N_SHELLS_DEFAULT

#: Folder-name format for bond reaction folders persisted by
#: :class:`autokmc2.io.persistence.ReactionWriter` (when enabled).

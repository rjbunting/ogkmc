...
* ``type``           – one of ``"bulk"``, ``"surface"``, ``"adsorbate"``,
  ...
  :func:`autokmc2.sites.anchors.find_anchor_sites` (one node per
  raw site clique).
...
``mult=autokmc2.core.constants.NL_MULT_DEFAULT``).
...
from autokmc2.core.constants import NL_MULT_DEFAULT
...
def build_graph(
    ...
    *,
    ...
    :func:`autokmc2.structure.tag_surface_atoms` or by calling
    :func:`autokmc2.structure.find_surface_atoms` with ``tag_atoms=True``.
    ...
    nl_mult : float
        ...
        :data:`autokmc2.core.constants.NL_MULT_DEFAULT` (currently ``1.0``).

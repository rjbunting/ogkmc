...
autokmc2.cli.main
===========
Command-line interface for the autokmc2 pipeline.

Two subcommands:

* ``autokmc2 run CONFIG``             — run the full pipeline.
* ``autokmc2 validate-config CONFIG`` — parse the config and exit.

The CLI is **calculator-agnostic** — see :class:`autokmc2.io.config.CalculatorCfg`
...
from autokmc2.io.calculators import build_calculator, calculator_meta
from autokmc2.utils.logging import get_logger
from autokmc2.io.persistence import ReactionWriter
...
def run_from_config(cfg: RunConfig, *, config_path: str | None = None) -> dict:
    """Run the autokmc2 pipeline end-to-end from a :class:`RunConfig`.

    Returns the KMC summary dict (same shape as
    :func:`autokmc2.kmc.engine.run_kmc_steps`) augmented with an
    ``outputs`` key listing the files written.
    """
    from autokmc2.structure import build_surface, build_nanoparticle
    ...
    from autokmc2.sites.diffusion import find_diffusion_sites
    from autokmc2.reactions.templates import derive_bond_templates
    from autokmc2.sites.bond import (
        find_bond_sites,
        prune_unstable_bond_sites,
        rebuild_bond_reverse_indexes,
    )
    from autokmc2.kmc.expansion import initialise_bond_registry
    from autokmc2.kmc.engine import run_kmc_steps
    from autokmc2.species.smiles import canonical_smiles
    ...
    from autokmc2.thermo.free_energy import FreeEnergyOptions
    ...
    if b.enabled:
        ...
        if log_level <= logging.INFO:
            print(
                f"[autokmc2] Bond reactions enabled: derived "
        ...
        if templates:
            ...
            for cs in leaves:
                if log_level <= logging.INFO:
                    print(f"[autokmc2]   leaf species: {cs!r} — building Reactant + adsorbate sites…")
        ...
        if templates:
            from autokmc2.sites.bond import _prune_one_per_adsorption_triple
        ...
        if log_level <= logging.INFO:
            print(
                f"[autokmc2] Bond reactions: {len(bond_sites)} "
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="autokmc2", description="autokmc2 CLI")
    p.add_argument(
        "--version",
        action="version",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.cmd == "run":
        ...
        print(f"\n[autokmc2] Run complete — output: {out_dir}")
        print(

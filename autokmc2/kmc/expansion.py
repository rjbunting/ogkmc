...
2. **Find adsorbate sites for C** — so the geometric enumerator
   :func:`autokmc2.sites.bond.find_bond_sites` has placements to
   match against.
...
The actual list of enumerated :class:`~autokmc2.sites.bond.BondReactionSite`'s
...
* :func:`expand_bond_sites_after_event`   — convenience hook that takes
  a fired :class:`~autokmc2.reactions.bond.BondReaction` and triggers expansion
...
from autokmc2.sites.adsorbate import AdsorbateSite, find_adsorbate_sites
...
from autokmc2.species.reactant import Reactant, build_reactant
...
def initialise_bond_registry(
    ...
    *,
    ...
    :func:`autokmc2.sites.bond.find_bond_sites` so subsequent calls to
    ...
    expanded_smiles : iterable[str] | None
        ...
        :func:`~autokmc2.reactions.templates.derive_bond_templates`, not merely
...
def expand_bond_sites_for_new_species(
    ...
    *,
    ...
    2. Build a :class:`~autokmc2.species.reactant.Reactant` and find
       :class:`~autokmc2.sites.adsorbate.AdsorbateSite`'s for it.
    ...
    5. Run :func:`autokmc2.sites.bond.find_bond_sites` for the new
    ...
    calculator
        ASE calculator handed to :func:`autokmc2.species.reactant.build_reactant`
        and :func:`autokmc2.sites.adsorbate.find_adsorbate_sites`.
...
def expand_bond_sites_after_event(
    ...
    **expand_kwargs,
    ...
    :class:`~autokmc2.reactions.bond.BondReaction` (``kind == "bond"``), triggers

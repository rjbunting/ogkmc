...
autokmc2.kmc.engine
======================
Kinetic Monte Carlo (BKL / Gillespie) engine for autokmc2.

This module mirrors the role of :mod:`disreax_kmc.simulation` but operates on
the on-the-fly :class:`~autokmc2.reactions.adsorption.AdsorptionReaction` objects produced from
materialised :class:`~autokmc2.sites.adsorbate.AdsorbateSite`'s.
...
from autokmc2.species.reactant import Reactant
from autokmc2.reactions.adsorption import (
...
from autokmc2.reactions.rates import KB_EV, H_EV_S, DEFAULT_TRANSMISSION_COEFFICIENT
from autokmc2.reactions.diffusion import (
    DiffusionReaction,
    compute_all_diffusions,
    get_applicable_diffusions,
)
from autokmc2.reactions.bond import (
    BondReaction,
    compute_all_bond_reactions,
    get_applicable_bond_reactions,
)
from autokmc2.kmc.expansion import expand_bond_sites_after_event
from autokmc2.kmc.execute import execute_reaction
from autokmc2.kmc.index import _ReactionIndex
from autokmc2.kmc.sampling import choose_reaction, sample_tau, total_rate
from autokmc2.kmc.state import (
...
from autokmc2.core.constants import LATERAL_SHELLS_DEFAULT
...
def run_kmc_steps(
    ...
    *,
    ...
    free_energy_options=None,
    vib_cache_root: str | None = None,
    # ── Optional persistence hooks (autokmc2.io.persistence) ──────────────────
    ...
    calculator
        ...
        :func:`~autokmc2.sites.stability.adsorption.check_site_stability`.
    reactants
        :class:`~autokmc2.species.reactant.Reactant`, iterable thereof, or
    ...
    diffusion_sites : list[DiffusionSite] | None
        ...
        :func:`autokmc2.sites.diffusion.find_diffusion_sites`.  When
    ...
    diffusion_kwargs : dict | None
        ...
        :func:`autokmc2.reactions.diffusion.get_applicable_diffusions` (NEB knobs:
    ...
    bond_sites : list[BondReactionSite] | None
        ...
        :func:`autokmc2.sites.bond.find_bond_sites`.  When non-empty
        the bond channel is enabled: every applicable
        :class:`~autokmc2.reactions.bond.BondReaction` (couple ``A+B→C`` and
    ...
    bond_kwargs : dict | None
        Keyword arguments forwarded to
        :func:`autokmc2.reactions.bond.get_applicable_bond_reactions` (NEB knobs,
        same names as *diffusion_kwargs*).
    bond_growth_kwargs : dict | None
        Keyword arguments forwarded to
        :func:`autokmc2.kmc.expansion.expand_bond_sites_after_event`,
    ...
    trajectory_writer : autokmc2.io.persistence.TrajectoryWriter | None
        Optional writer.  Initial state plus every Nth state is written to
        an ASE ``.traj`` (cadence configured on the writer itself).
    summary_collector : autokmc2.io.persistence.ReactionSummary | None

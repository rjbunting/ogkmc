"""
autokmc2.sites.diffusion
============================
Enumerate diffusion (hop) site-pairs on top of materialised
:class:`~autokmc2.sites.adsorbate.AdsorbateSite`'s.
...
* a **surface-graph link** between their bonded surface cliques: the minimum
  ...
  most ``max_hops`` (default :data:`~autokmc2.core.constants.DIFFUSION_MAX_HOPS`
...
:func:`autokmc2.sites.adsorbate.prune_unstable_adsorbate_sites`, so every
endpoint is known stable.  Lateral-environment classification and the NEB
transition-state search are deferred to
:mod:`autokmc2.sites.stability.diffusion` and run lazily during the KMC loop —
...
class DiffusionLateral:
    ...
    Mirrors :class:`autokmc2.sites.adsorbate.AdsorbateSiteLateral` for
    the diffusion case.  Energies and relaxed atoms are filled in lazily by
    :func:`autokmc2.sites.stability.diffusion.check_diffusion_stability` once the
    ...
    ego_graph : nx.Graph | None
        ...
        :func:`autokmc2.sites.stability.diffusion.check_diffusion_site_lateral`.
    n_shells : int
...
def _member_clique_union(site: AdsorbateSite, m_idx: int) -> frozenset[int]:
    ...
    by :func:`~autokmc2.sites.adsorbate.find_adsorbate_sites`.
    """
...
def _build_pair_ego_graph(
    ...
    ego-graph (:func:`autokmc2.sites.stability.adsorption._build_lateral_ego_graph`).
...
def find_diffusion_sites(
    ...
    *,
    ...
    max_hops : int
        ...
        :data:`~autokmc2.core.constants.DIFFUSION_MAX_HOPS` (= 1).
    n_shells_pair : int
        Surface-only BFS depth used for iso-class deduplication.
    surface_apsp_cutoff : int
        Cutoff handed to :func:`autokmc2.sites.adsorbate._get_surface_apsp`

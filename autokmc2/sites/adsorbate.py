"""
autokmc2.sites.adsorbate
============================
N-atom adsorbate site enumeration on a surface graph, built cleanly on top of
:mod:`autokmc2.sites.anchors` and :mod:`autokmc2.species.reactant`.

Where :func:`~autokmc2.sites.anchors.find_anchor_sites` finds all single-atom
adsorption *anchor* sites for one element, this module finds all geometrically
feasible placements of an arbitrary :class:`~autokmc2.species.reactant.Reactant`
...
3. **Backtracking chain placement.**  For every orbit-canonicalised non-empty
   ...
   * The first anchor is placed at every raw anchor-node position of its
     ...
     :func:`~autokmc2.sites.anchors.find_anchor_sites`).
...
5. **Reduce by isomorphism.**  Group placements by isomorphism of the union-
   ...
   :func:`~autokmc2.sites.anchors._build_ego_graph`).
...
# ---------------------------------------------------------------------------
# Tuneable defaults — single source of truth in :mod:`autokmc2.core.constants`.
...
class AdsorbateSiteLateral:
    ...
    :func:`autokmc2.sites.stability.adsorption.check_adsorbate_site_lateral` — one
    ...
    ego_graph : nx.Graph | None
        ...
        :func:`autokmc2.sites.stability.adsorption._build_lateral_ego_graph`.
    ...
    #: set by :func:`autokmc2.sites.stability.adsorption.check_site_stability`.
    ...
    #: ``None`` until :func:`autokmc2.sites.stability.adsorption.check_site_stability`
    #: has been run successfully.
    stable            : bool | None  = None
    #: Relaxed ASE :class:`~ase.Atoms` snapshot of the **occupied** state used
    #: in :func:`autokmc2.sites.stability.adsorption.check_site_stability` (i.e. the
    #: structure that produced ``energy_occupied``).  Persisted by
    #: :class:`autokmc2.io.persistence.ReactionWriter` as ``occupied.extxyz``
...
class AdsorbateSite:
    ...
    #: :func:`autokmc2.sites.stability.adsorption.check_adsorbate_site_lateral`).
...
def _ensure_anchor_sites(
    ...
    *,
    ...
    """Lazily run :func:`~autokmc2.sites.anchors.find_anchor_sites` if needed."""
...
def _all_raw_sites_with_positions(
    ...
    :func:`~autokmc2.sites.anchors.find_anchor_sites` — one per raw clique,
...
def _build_pruning_atoms(
    ...
    *,
    ...
    array that mirrors :mod:`autokmc2.core.graph` conventions (0=bulk, 1=surface,
    2=adsorbate) so that :func:`autokmc2.core.graph.build_graph` can be called
...
def _intended_adsorbate_edges(
    ...
    * Anchor bonds: for every reactant atom *i* with
    ...
    edges actually produced by :func:`autokmc2.core.graph.build_graph` on the
...
def prune_unstable_adsorbate_sites(
    ...
    *,
    ...
    1. Build a bare slab + adsorbate :class:`~ase.Atoms` from the
       ...
       ``surface == 2`` so :func:`autokmc2.core.graph.build_graph` can be called
       on it directly.
    2. Run a full ML relaxation via
       :func:`~autokmc2.structure.optimise_structure`.
    ...
    ----------
    G : nx.Graph
        Surface graph from :func:`~autokmc2.core.graph.build_graph`.
    adsorbate_sites : list[AdsorbateSite]
    ...
    fmax : float
        ...
        :data:`~autokmc2.core.constants.PRUNE_FMAX`.
    max_steps : int
        Maximum LBFGS steps.  Default
        :data:`~autokmc2.core.constants.PRUNE_MAX_STEPS`.
    nl_mult : float
        Neighbour-list cutoff multiplier handed to
        :func:`autokmc2.core.graph.build_graph` for the connectivity comparison.
        Default :data:`~autokmc2.core.constants.NL_MULT_DEFAULT` — matches the
    ...
    """
    from autokmc2.structure import optimise_structure  # avoid circular at module level
...
def find_adsorbate_sites(
    ...
    *,
    ...
    ----------
    G : nx.Graph
        Surface graph from :func:`autokmc2.core.graph.build_graph`.  Anchor sites
    ...
    co_factor, opt_factor, repulsion_weight :
        Forwarded to :func:`~autokmc2.sites.anchors.find_anchor_sites` if
    ...
    prune_fmax : float
        ...
        Default :data:`~autokmc2.core.constants.PRUNE_FMAX` (0.05).
    prune_max_steps : int
        Maximum LBFGS steps for pruning relaxations.
        Default :data:`~autokmc2.core.constants.PRUNE_MAX_STEPS` (200).
    verbose : bool
...
def optimise_adsorbate_site_positions(
    ...
    *,
    ...
    :func:`~autokmc2.sites.anchors._kabsch_align_ego`.

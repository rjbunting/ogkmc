"""
autokmc2.sites.stability.bond
========================
On-the-fly lateral-interaction classifier and CI-NEB stability / barrier
calculator for materialised :class:`~autokmc2.sites.bond.BondReactionSite`'s.

This is the bond-reaction analogue of :mod:`autokmc2.sites.stability.diffusion`.
Each :class:`~autokmc2.sites.bond.BondReactionSite` enumerates an
...
2. The **two endpoint relaxations** for that lateral class — one ML
   ...
   :class:`~autokmc2.sites.bond.BondReactionLateral`.
...
Lateral ego-graph conventions (extends :mod:`autokmc2.sites.stability.adsorption`)
---------------------------------------------------------------------------
...
* (Re-exported) :class:`SurfaceConnectivityError`,
  ...
  :mod:`autokmc2.sites.stability.adsorption`.
...
from autokmc2.sites.bond import BondReactionSite, BondReactionLateral
...
# ---------------------------------------------------------------------------
# Errors  (mirror autokmc2.sites.stability.diffusion)
# ---------------------------------------------------------------------------

class BondStabilityError(Exception):
    """Base class for all bond-reaction stability / NEB failures.

    Caught by :func:`autokmc2.reactions.bond.get_applicable_bond_reactions` and
...
def _bond_lateral_node_match(d1: dict, d2: dict) -> bool:
    ...
    Like :func:`autokmc2.sites.stability.diffusion._diffusion_lateral_node_match`
...
def _build_bond_lateral_ego_graph(
    ...
    *,
    ...
    Mirrors :func:`autokmc2.sites.stability.diffusion._build_diffusion_lateral_ego_graph`
...
def check_bond_site_lateral(
    ...
    *,
    ...
    clq_c = _member_clique_union(site_c, m_c)
    if not clq_a or not clq_b or not clq_c:
        raise ValueError(
...
def _build_bond_atoms(
    ...
    *,
    ...
    if len(a_ordered) + len(b_ordered) != len(c_present):
        raise ValueError(
...
def _make_neb_band(
    ...
    *,
    ...
    neb = NEB(images, k=float(spring_k), climb=bool(climb))
...
def _check_bond_ts_validity(
    ...
    *,
    ...
    Mirrors :func:`autokmc2.sites.stability.diffusion._check_ts_validity` with
...
def check_bond_site_stability(
    ...
    *,
    ...
    Pipeline (mirrors :func:`autokmc2.sites.stability.diffusion.check_diffusion_stability`):

"""
autokmc2.sites.stability.adsorption
=============================
...
1. Call :func:`check_adsorbate_site_lateral` for any specific member of an
   :class:`~autokmc2.sites.adsorbate.AdsorbateSite`.
...
3. The ego-graph is compared by isomorphism to every
   :class:`~autokmc2.sites.adsorbate.AdsorbateSiteLateral` already stored
   on the parent :class:`~autokmc2.sites.adsorbate.AdsorbateSite`.
...
5. The matching (or newly created)
   :class:`~autokmc2.sites.adsorbate.AdsorbateSiteLateral` is returned.
...
:func:`~autokmc2.structure.optimise_structure`.  After each relaxation the
...
Energies are stored on the :class:`~autokmc2.sites.adsorbate.AdsorbateSiteLateral`
...
from autokmc2.core.constants import NL_MULT_DEFAULT, LATERAL_SHELLS_DEFAULT
...
def check_adsorbate_site_lateral(
    ...
    *,
    ...
    against every :class:`~autokmc2.sites.adsorbate.AdsorbateSiteLateral`
    ...
    G : nx.Graph
        ...
        :func:`~autokmc2.core.graph.build_graph` after
        :func:`~autokmc2.sites.adsorbate.find_adsorbate_sites` and with
    ...
    n_shells : int | None
        ...
        :data:`~autokmc2.core.constants.LATERAL_SHELLS_DEFAULT` (``0``), meaning
...
def _expand_to_full_placement(G: nx.Graph, seed_node_ids: set[int]) -> set[int]:
    ...
    :func:`~autokmc2.sites.adsorbate._materialise_adsorbate_nodes` so
...
def _bond_set(
    ...
    *,
    ...
    Uses the same ``natural_cutoffs`` scheme as :func:`~autokmc2.core.graph.build_graph`
...
def check_site_stability(
    ...
    * **unoccupied** — same slab + lateral neighbours, site absent.
    ...
    :func:`~autokmc2.structure.optimise_structure` (LBFGS, deep-copied
    ...
    nl_mult : float
        ...
        Default :data:`~autokmc2.core.constants.NL_MULT_DEFAULT`.
    verbose : bool
    ...
    """
    from autokmc2.structure import optimise_structure  # local import avoids circular
    ...
    # Persisted later by autokmc2.io.persistence.ReactionWriter as
    ...
    if (free_energy_options is not None
        ...
        from autokmc2.thermo.free_energy import compute_harmonic_thermo

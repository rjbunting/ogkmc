"""
autokmc2.species.bond_chemistry
===============================
...
* If *smiles_or_atoms* is an ASE :class:`~ase.Atoms` object, connectivity is
  ...
  :func:`autokmc2.core.graph.build_graph`; bond orders default to ``SINGLE``.
"""
...
from autokmc2.core.constants import NL_MULT_DEFAULT, RANDOM_SEED
...
class FragmentPair:
    ...
    reactant_a : Reactant or None
        :class:`autokmc2.species.reactant.Reactant` for fragment A; populated only
        when :func:`get_all_fragments` is called with ``as_reactants=True``.
    reactant_b : Reactant or None
        :class:`autokmc2.species.reactant.Reactant` for fragment B.
    """
...
def _rdkit_mol_from_atoms(atoms):
    ...
    (ASE / autokmc2 graphs do not track bond orders).  Positions are taken
    directly from *atoms*.
    """
    try:
        from rdkit import Chem
...
def fragment_smiles_to_atoms(smiles: str, *, add_hydrogens: bool = True):
    ...
    """
    from autokmc2.species.reactant import _smiles_to_atoms
...
class CombinedSpecies:
    ...
    reactant : Reactant or None
        :class:`autokmc2.species.reactant.Reactant` for the combined species;
...
def combine_fragments(
    ...
    **Round-trip: break CO double bond, recombine**::
        ...
        >>> from autokmc2.species.bond_chemistry import get_all_fragments, combine_fragments

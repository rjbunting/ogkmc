"""
autokmc2.reactions.diffusion
=====================
Build KMC diffusion (hop) reactions for a graph of materialised
:class:`~autokmc2.sites.diffusion.DiffusionSite`'s.

Mirrors :mod:`autokmc2.reactions.adsorption` for adsorption / desorption.  For
...
3. **Lateral classification** — :func:`autokmc2.sites.stability.diffusion.
...
4. **NEB stability** — the first time a member maps to a new lateral class,
   :func:`autokmc2.sites.stability.diffusion.check_diffusion_stability` relaxes
...
5. **Energetics** for the firing direction::
   ...
   reaction folder by :class:`autokmc2.io.persistence.ReactionWriter`; the
   ``EA_MIN`` floor is only applied when computing the rate consumed by
   the KMC loop, exactly as documented on
   :data:`autokmc2.reactions.adsorption.EA_MIN`.
...
from autokmc2.sites.diffusion import DiffusionSite, DiffusionLateral
...
def fast_diffusion_for_member(
    ...
    *,
    ...
    .. warning::
       ...
       through :func:`autokmc2.kmc.engine._recompute_affected_sites`.
